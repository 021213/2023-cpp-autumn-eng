...
class BiLinkedList {
public:
	BiLinkedList() : head(nullptr), length(0) {}
	BiLinkedList(const BiLinkedList& list) : head(nullptr), length(list.length)
	{
		head = copy(list.head);
	}
...
private:
	...
	BiNode<T>* ExtractBiNode(int index)  //todo:check and fix
	{
	...
	BiNode<T>* copy(BiNode<T>* src)  //todo:check and fix
	{
		...
		while (src != nullptr)
		{
			dst->next = new BiNode<T>(src->data);
			dst = dst->next;
			src = src->next;
		}
	}
